...
namespace AZEM.iOS.Renderers 
{
    public class  EntryUnderline: EntryRenderer
    {
        ...
        {
            if (e.PropertyName == EntryUnderlineControl.WidthProperty.PropertyName)
            {
                bottomBorder?.RemoveFromSuperLayer();
            }
            if (e.PropertyName == EntryUnderlineControl.HeightProperty.PropertyName)
            {
                bottomBorder?.RemoveFromSuperLayer();
            }
            if (e.PropertyName == EntryUnderlineControl.EntryColorProperty.PropertyName)
            {
                bottomBorder?.RemoveFromSuperLayer();
                controlColor = formControl.EntryColor.ToCGColor();
            }
            if (e.PropertyName == EntryUnderlineControl.HasErrorProperty.PropertyName)
            {
            ...
            {
                var returnType = (sender as EntryUnderlineControl).ReturnKeyType;
                if (this.Element.Keyboard == Keyboard.Numeric || this.Element.Keyboard == Keyboard.Telephone)
                {
                    this.AddAccessoryButton(returnType);
                }
